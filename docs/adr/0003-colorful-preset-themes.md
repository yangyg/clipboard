...
首发仅暗色基调，随后补充三个浅色变体：`dracula-light`（紫霞）/ `nord-light`（冰白）/ `sunset-light`（暖阳），与对应暗色家族共享 hue 与 accent 色相，但 token 整组按浅底精调（文字/类型/语义色加深以保证 AA）。浅色变体同样是**固定主题**，`system` 永不解析到它们。侧栏「深浅切换」把浅色彩色主题归入「亮」侧（浅彩色 → dark，深彩色/oled/system/dark → light）。
