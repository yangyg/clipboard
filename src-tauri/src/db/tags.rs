...
//! Standalone tags.json merge lives in `tags_sync.rs`.
...
// `pub(super)` so the split-out `tags_tests` module can still exercise it.
pub(super) fn normalize_color_key(color: &str) -> String {
...
impl ClipboardDb {
    ...
    /// Caller must already hold the write connection (insert path shares one
    /// transaction). Returns the tag names that were linked.
    pub(super) fn apply_auto_tags_on(
        &self,
        conn: &Connection,
        record_id: i64,
        content: &str,
        ...
        if !settings.features.tags || !settings.enable_auto_tag {
            return Ok(Vec::new());
        }
        ...
        for (tag_name, lower_keywords, content_types) in &matcher.rules {
            ...
            if (type_hit || keyword_hit) && !matched.iter().any(|n| n == tag_name) {
                matched.push(tag_name.clone());
            }
        }
        if matched.is_empty() {
            return Ok(matched);
        ...
        for tag_name in &matched {
            let tag_id = Self::ensure_auto_tag_conn(conn, tag_name)?;
            conn.execute(
                "INSERT OR IGNORE INTO record_tags (record_id, tag_id) VALUES (?, ?)",
                params![record_id, tag_id],
            )?;
    ...
    pub fn apply_auto_tags(
        ...
        self.apply_auto_tags_on(&tx, record_id, content, content_type, settings)?;
        tx.commit()?;
        Ok(())
    }
