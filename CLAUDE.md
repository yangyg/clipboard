...
- **Auto-tag:** Settings `enable_auto_tag` (default **true**) + `auto_tag_rules`. Per-rule match is OR. Applied in a **single DB transaction** (batch `record_tags`). Defaults: 链接←`link`; 部署 / 前端←keyword lists. UI under Settings → 标签. Frontend `scheduleLoadTags` (350ms) coalesces `get_all_tags` after filter/tag bursts. List soft-cap also applies after `loadMore`.
