...
- **Preview chrome:** Type + actions in header; source / time / size-or-chars / 富文本 / 使用次数 as one meta line (`title` = content type). Single scroll on `.preview-content` (no nested scroll on content / rich HTML).
