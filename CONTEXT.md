...
| **Keyset pagination** | List queries use keyset cursors (`before_pinned` / `before_updated_at` / `before_id`, plus `before_created_at` / `before_copy_count` for those sorts) instead of OFFSET to avoid drift when the list mutates. |
| **Soft cap** | In-memory list pages are soft-capped (`PAGE_SIZE × 2`). Trimmed tail stays in the DB; the next `loadMore` continues from the last remaining row via keyset. |
...
- **Settings persist APIs** — one `app_settings` JSON blob. Semantic writers: `save_settings` (user IPC, runs OS side effects), `save_window_geometry`, `save_sync_metadata` (WebDAV stamps; overlays onto a fresh load), `ensure_device_identity`. Autostart / shortcut / chrome live in `settings_effects` and run only on the user save path.
- **Single SQLite writer** — `ClipboardDb::lock_write()` is the only write-lock entry (WAL allows one writer). Search `lock_write` for contention; do not add a second write `Connection`.
