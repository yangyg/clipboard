...
- **Theming / tokens:** CSS vars on `:root` (incl. `--type-*`, `--pin` / `--pin-soft`, `--text-xs`…`--text-xl`, `--space-*`, `--win-close-hover`). Themes: `.light-theme` / `.oled-theme`. SideBar can toggle dark↔light.
  ...
  - **Type colors (`--type-*`):** text sky `#7dd3fc` · code green `#34d399` · link deep blue `#2563eb` · image cyan `#0ea5e9` · file amber `#eab308`. Badges (`.badge-*`), SideBar category active (`--cat-color`), and type icons / link titles follow these. List **selection/hover** is Fluent flat (accent soft fill / `--bg-hover`), not type-colored card borders.
...
- **Preview chrome:** Type + actions in header; source / time / size-or-chars / 富文本 / 粘贴次数 as one meta line. **Borders:** keep header bottom + actions top dividers only; text body has no box border; link/file use elevated fill without stroke; image thumb uses a hairline outline for contrast. Image preview: click → `open_record_media` (canonicalize under media root; Windows `ShellExecuteW` — not `cmd /c start` / `shell.open`). Preview links only allow `http:` / `https:`.
